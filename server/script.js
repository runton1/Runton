...
// Пассивный доход в коллектор пока человек в игре (offline-like tick раз в 1 мин)
setInterval(() => {
  const earned = calcOfflineIncome(60); // 60 секунд = 1 минута
  if (earned > 0){
...
async function syncFromServer(){
  try {
    ...
    // Загружаем коллектор с сервера (накопленное не сбрасываем)
    if (u.collectorPending > 0){
      // Берём максимум из локального и серверного коллектора
...
function showOfflineModal(amount){
  ...
  // Оффлайн доход попадает в коллектор, не в верхний баланс
