...
// Метров между монетками (COIN_SPACING/BASE_SPEED * distPerFrame)
// = (W*0.38)/(W*0.004) * (W*0.004/W * 8) = 95 * 0.032 = 3.04 м/монету
const METERS_PER_COIN = (0.38 / 0.004) * (0.004 * 8); // = 3.04

// Стоимость монетки для локации (TON/монету)
function coinValueForLoc(id){
  const inc = locIncome[id];
  if (!inc || inc.expired) return 0;
  return (inc.totalTon / METERS_36) * METERS_PER_COIN;
}
...
// Ставка активной локации TON/сек — та же что даёт монетка.
// coinValue / METERS_PER_COIN = TON/метр; * METERS_PER_SEC = TON/сек.
// Онлайн монетка даёт coinValue, оффлайн accrueIncome считает по этой же ставке.
function ratePerSecond(id){
  const inc = locIncome[id];
  if (!inc || inc.expired) return 0;
  return (inc.totalTon / METERS_36) * METERS_PER_SEC;
}
...
function collectCoin(c){
  ...
  // Реальный доход от монетки — та же ставка что и оффлайн
  const value = coinValueForLoc(Game.currentLoc)
              * (Game.skinBonus > Date.now() ? 1.05 : 1);
  if (value > 0){
    Game.pendingCoins += value;
    // Синхронизируем lastTick: accrueIncome не будет двоить,
    // т.к. монетка уже начислила доход за этот момент.
    // Просто обновляем lastTick чтобы не было перекрытия.
    Game.lastTick = Date.now();
    updateCollector();
    floatTexts.push({ text:'+'+value.toFixed(6), x:c.x, y:COIN_Y-COIN_R*2,
                      color:'#ffe066', size:W*0.028, life:1 });
  }
